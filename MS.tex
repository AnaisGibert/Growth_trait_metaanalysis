...
\item Despite a sizeable literature on traits and growth, major knowledge gaps across size-classes still exist; for one-third of traits a majority of correlations with growth reported in the literature ($>$ 75\%) come from a single and unique size-classe. Clearly, there is scope for strengthening our understanding on the link between traits and growth through adding further empirical studies for traits and size-classes underrepresented in the literature.
...
Despite a sizeable literature on traits and growth, there are major knowledge gaps on the link between trait and growth. The number of correlations reported for each 19 traits is generally unbalanced in favor of a particular stage, for historical or practical reasons. This means that the knowledge we have on this trait is often biased toward a particular size-classe. Additional data are specifically required in small plants (seedling and sapling) for WD and Hmax, and in large plants (adult and sapling) for Aarea and more broadly for all traits requiring a measure of whole plant biomass (e.g. LMR, LAR). 
