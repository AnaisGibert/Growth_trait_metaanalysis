...
strategy of the plant. We assume it is unrelated to the traits considered in the current paper, in other words that it does not automatically change in response to changes in the traits. The second term, $\ud A_{\rm l} / \ud M_{\rm a} $,
...
Plants with high SLA can deploy a unit of leaf area more cheaply. However, high-SLA species also have shorter leaf lifespan, and must use more energy replacing leaves lost via turnover \citep{Wright:2004jb}. These two effects occur within the terms  $\frac{\ud B}{\ud t}$ (net live biomass production) and $\frac{\ud A_{\rm l}}{\ud M_{\rm a} }$ (marginal cost of deploying additional leaf area), appearing on the right side in the equations for growth rate (\ref{eq:eq1}, \ref{eq:eq2}, \ref{eq:eq03}). Whether plant growth rate increases with SLA depends on how the relative magnitude of the benefit (cheap leaf construction, via $\frac{\ud A_{\rm l}}{\ud M_{\rm a} }$ ) and cost (higher leaf turnover, via $\frac{\ud B}{\ud t}$) of increased SLA changes with size (Table \ref{tab:trade-offs}).
...
\vspace{1cm}
  ...
  & & \tabitem{Absolute} & $\ud M_{\rm a} / \ud t$ & $\downarrow$\sepp$\downarrow$ & $-$\sepp$-$ &  $-${\sepp}$\uparrow$ & $\uparrow${\sepp}$-$ & $\uparrow${\sepp}$\uparrow$ \\
