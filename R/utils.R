...
# Imported from Hmisc to save loading that package
# ‘wtd.mean’, ‘wtd.var’ compute weighted means, and variances
# For full info see package ?Hmisc::wtd.mean
...
# Count records in data set
# Behaves similarly to plyr::count 
...
# Renames columns of data frame
# imported and modified from plyr::rename
rename <- function (data, replace, warn_missing = TRUE, warn_duplicated = TRUE) {
    ...
    if (warn_duplicated && (length(duplicated_names) > 0L)) {
        duplicated_names_message <- paste0("`", duplicated_names, 
            "`", collapse = ", ")
        warning("The rename operation has created duplicates for the ", 
