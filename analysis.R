...
figure_A9(GCi)
dev.off()
`combined_bib{1}` <- merge_bib_files(meta_bib, paper_bib)
combined_bib <- merge_bib_files(`combined_bib{1}`, read_bib)
...
figure_A5(RIi, RCi)
dev.off()
latex_build("ms-suppinfo.tex", bibliography = "output/refs.bib", clean = TRUE)
